...
// DB 인덱스 (cells 배열 원래 순서):
// 결과(0) 상태(1) 찾은날짜(2) 구두만픽(3) 경로(4) 유입(5) 나이(6) 이름(7)
// 거주지(8) 번호(9) 환경/정보(10) 티엠(11) 횟수(12) 날짜(13) 예약일시(14)

// 화면 표시 순서 → DB 인덱스 매핑 (COLS[화면순서] = DB인덱스)
const COLS = [
  { label: "결과",      di: 0                         },
  { label: "상태",      di: 1                         },
  { label: "구두만픽",  di: 3                         },
...
// sticky 컬럼의 left 오프셋 (이름=0, 번호=72, 티엠=182)
const STICKY_LEFT = (() => {
  let off = 0; const m = {};
...
// 아래는 모두 DB 인덱스 기준
const DATETIME_DI = 14;  // 예약일시
const READONLY_DIS = new Set([1, 2, 3, 5, 12, 13]); // 상태, 찾은날짜, 구두만픽, 유입, 횟수, 날짜
const TEXTAREA_DI = 10;  // 환경/정보

// TM 데이터 형식: "이름|MM/DD" 토큰을 공백으로 구분. 구형 데이터(파이프 없음)도 호환.
...
// 찾은날짜 "YYYY-MM-DD HH:MM" or "YYYY-MM-DD" → "MM/DD" 표시
...
function getOptConfig(ci, value) {
  if (!value) return null;
  return (DD_OPTS[ci] || []).find((o) => o.value === value) || null;
}

function applyRowLogic(cells, ci) {
  const c = cells.slice();
  const result = c[0] || "";
  const beonho = c[9] || "";      // 번호: DB index 9
  const yeyakIlsi = c[14] || ""; // 예약일시: index 14
  ...
  // 번호(DB9) 입력 AND 결과 미설정 → 미진행/진행가능
  if (ci === 9 && beonho && !result) {
    c[0] = "미진행";
    c[1] = "진행가능";
    return c;
  }

  // 티엠(11) 입력 → 횟수(12) = 엔트리 수, 날짜(13) = 최신 엔트리 날짜
  if (ci === 11) {
  ...
  // 횟수(12) 직접 수정 → 날짜(13)에 현재 시각
  if (ci === 12) {
    const d = new Date();
    c[13] = `${p(d.getMonth() + 1)}/${p(d.getDate())} ${p(d.getHours())}:${p(d.getMinutes())}`;
  }

  // 결과(0) 또는 예약일시(14) 변경 시 상태 자동 결정
  if (ci === 0 || ci === 14) {
    if (TERMINATE_RESULTS.has(result)) {
      c[1] = "최종종료";
    } else if (result === "티엠예약") {
      c[1] = yeyakIlsi ? "진행가능" : "확정일 기입 필요";
    } else if (result === "장기관리") {
      c[1] = "장기관리대상";
    } else if (result === "리트") {
      c[1] = "진행가능";
...
const nameZoneMap = ref({}); // { '이름': '3' } — 마운트 시 list-names API로 구성

function zoneBg(val) {
  const s = String(val || '').trim();
  const numMatch = s.match(/^(\d+)/);
  if (numMatch) return ZONE_COLORS[numMatch[1]] || '';
  // 숫자 없이 이름만 있으면 nameZoneMap에서 조회
  const zone = nameZoneMap.value[s];
  return zone ? (ZONE_COLORS[zone] || '') : '';
}
function cellBg(dbIdx, cells) {
  if (dbIdx === 5) return zoneBg(cells[5]);   // 유입 (DB5)
  if (dbIdx === 11) return zoneBg((cells[11] || '').split(/\s+/)[0].split('|')[0]); // 티엠 (DB11)
  return '';
}
...
// ── 헤더 필터 ─────────────────────────────────────────────────────────
// colFilters[ci] = Set<string> (허용값). 없으면 필터 없음.
const colFilters = ref({});
const filterPopup = ref(null); // { ci, x, y }
...
// 드롭다운: 아래 공간 280px 미만이면 위로 열기 (transform 방식)
...
async function load() {
  loading.value = true;
  try {
    const r = await callApiPromise("/api/tm-sheet/rows", {});
    if (!r.success) { showAppAlert("시트 조회 실패: " + (r.message || "")); loading.value = false; return; }
    rows.value = (r.rows || []).map((row) => ({ id: row.id, cells: padCells(row.cells) }));
    expireStaleInflows();
    const targetId = Number(route.query.rowId);
    if (targetId) {
      nextTick(() => {
        const idx = rows.value.findIndex((row) => row.id === targetId);
        if (idx < 0) return;
        highlightedRowIdx.value = idx;
        const tr = document.querySelector(`tr[data-row-id="${targetId}"]`);
        tr?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      });
    }
  } catch (e) {
    showAppAlert("시트 조회 중 오류: " + e.message);
  }
  loading.value = false;
}

// 유입(DB5) 48시간 실효 처리 — 찾은날짜(DB2) 기준, 날짜만 있는 행은 자정 기준
function expireStaleInflows() {
  const now = Date.now();
  const MS_48H = 48 * 3600 * 1000;
  rows.value.forEach((row, idx) => {
    const inflow = (row.cells[5] || '').trim();
    if (!inflow || inflow === '—') return;
    const dateStr = (row.cells[2] || '').trim();
    if (!dateStr) return;
    // "YYYY-MM-DD HH:MM" 또는 "YYYY-MM-DD"
    const isoStr = dateStr.length === 10
      ? dateStr + 'T00:00:00+09:00'
      : dateStr.replace(' ', 'T') + ':00+09:00';
    const dt = new Date(isoStr);
    if (isNaN(dt.getTime())) return;
    if (now - dt.getTime() > MS_48H) {
      rows.value[idx].cells[5] = '—';
      saveRowDeferred(idx, rows.value[idx].cells.slice());
    }
  });
}
...
function saveRowDeferred(rowIdx, cells) {
  ...
  pendingSave.set(row.id, setTimeout(async () => {
    ...
    try {
      const r = await callApiPromise("/api/tm-sheet/save-row", { id: row.id, cells: cellsCopy });
      if (!r.success && isMounted) showToast("저장 실패: " + (r.message || ""));
    } catch (e) {
...
function confirmDeleteRow(rowIdx) {
  ...
  showAppConfirm(`${rowIdx + 1}번째 행을 삭제할까?`, async (yes) => {
    ...
    saving.value = true;
    try {
      const r = await callApiPromise("/api/tm-sheet/delete-row", { id: row.id });
      if (!r.success) { showAppAlert("행 삭제 실패: " + (r.message || "")); saving.value = false; return; }
      rows.value.splice(rowIdx, 1);
      showToast("행이 삭제됐어!");
    } catch (e) {
      showAppAlert("행 삭제 중 오류: " + e.message);
    }
    saving.value = false;
  });
}
...
function connectSSE() {
  ...
  const url = token && tokenStore.isWebApp()
    ? `/api/tm-sheet/events?token=${encodeURIComponent(token)}`
    : '/api/tm-sheet/events';
  sseSource = new EventSource(url);
  sseSource.addEventListener('reload', () => {
    if (!editCell.value && !tmPopup.value) load();
  });
  sseSource.onerror = () => {
    sseSource.close();
...
<template>
  <div class="sv-screen">
    <!-- 상단바 -->
    <div class="sv-topbar">
      <span class="sv-title">오프라인 시트</span>
    ...
    <div v-else class="sv-wrap">
      <table class="sv-table">
        <thead>
          <tr>
            <th v-for="(col, ci) in HEADERS" :key="ci" class="sv-th" :class="[colGroup(ci), { 'sv-th-filtered': colHasFilter(ci), 'sv-td-narrow': ci === 2, 'sv-td-addr': ci === 6, 'sv-col-name': ci === 4, 'sv-col-beonho': ci === 7 }]" :style="COLS[ci]?.sticky ? { position: 'sticky', left: STICKY_LEFT[ci] + 'px', zIndex: 3 } : {}" @click.stop="openFilterPopup(ci, $event)">{{ col }}<span class="sv-th-fbtn" :class="{ 'sv-th-fbtn-on': colHasFilter(ci) }">▾</span></th>
        ...
        <tbody @click.stop="handleTableClick" @mouseover="onTbodyMouseover" @mouseleave="onTbodyMouseleave">
          <tr
            ...
            :class="{ 'sv-row-highlighted': row._origIdx === highlightedRowIdx }"
            :data-row-id="row.id"
            :style="rowBg(row.cells[0]) ? { '--row-bg': rowBg(row.cells[0]) } : {}"
          >
            <td
              v-for="(_, ci) in HEADERS" :key="ci" class="sv-td"
              :class="{ 'sv-td-ro': READONLY_DIS.has(di(ci)), 'sv-td-center': ci === 2 || ci === 3 || ci === 12, 'sv-td-narrow': ci === 2, 'sv-td-addr': ci === 6, 'sv-col-name': ci === 4, 'sv-col-beonho': ci === 7 }"
            ...
            >
              ...
              <!-- 구두만픽(화면2=DB3): 잡은 것만 체크, 나머지 빈칸 -->
              <template v-else-if="ci === 2">
                <span v-if="row.cells[3] === 'O'" class="sv-vmf-o">✓</span>
              </template>
              <!-- 찾은날짜(화면3=DB2): MM/DD 표시 -->
              <template v-else-if="ci === 3">{{ fmtFoundDate(row.cells[2]) }}</template>
              <!-- 티엠(화면11=DB11): 이름+뱃지 -->
          ...
          <tr v-if="!rows.length">
            <td :colspan="HEADERS.length + 1" class="sv-empty-row">일일보고에서 번호찾을 등록하면 자동으로 추가돼!</td>
          </tr>
...
.sv-table { border-collapse: separate; border-spacing: 0; font-size: 12px; font-family: "Noto Sans KR",Arial,sans-serif; background: var(--card-bg,#fff); width: 100%; }
...
.sv-row-highlighted .sv-td { background: #FFF9C4 !important; }
...
/* 구두만픽 */
.sv-vmf-o { color: #2E7D32; font-weight: bold; font-size: 15px; }
/* 너비 제한 */
