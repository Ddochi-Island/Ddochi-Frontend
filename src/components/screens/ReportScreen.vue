...
<template>
  <div class="report-screen">
    ...
    <template v-else-if="periodData">
      <!-- Table 1: 발생 기준 -->
      <div class="section-title">📊 발생 기준</div>
      <div class="section-desc">* 이번 기간 동안 <b>새로 발생한</b> 번호찾과 재가 건수입니다.</div>
          ...
          <tbody>
            <template v-for="g in sortedGroups" :key="'snap-' + g">
              <template v-if="getSnapshotData(g)">
                ...
                <tr :style="isTotalRow(g) ? 'background:#FFFDE7; font-weight:bold;' : ''">
                  <td style="font-weight: bold; color: #4CAF50;">재가</td>
      ...
      <!-- Table 2: 방어 기준 -->
      <div class="section-title">⚔️ 방어 기준</div>
      <div class="section-desc">* 이번 기간 내에 <b>약속이 잡혀 있던 만픽 인원(미정 제외)</b>을 얼마나 방어(매칭)했는지 보여줍니다.</div>
      <div class="stats-table-wrapper">
        <table class="stats-table">
          <thead>
            <tr>
    ...
    <div v-else style="text-align:center; padding:50px; color:#888;">
      ...
      <div style="font-weight:bold; margin-bottom:8px;">데이터를 불러오는 중이야!</div>
